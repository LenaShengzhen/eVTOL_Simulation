...
void Simulation::start_thread() {
    ...
    const int thread_num = numPlane + 1;

    // init the rlist before starting Thread
    init_rlist_before_startingThread(thread_num);
    ...
    // clean all Hazard pointer and rlist after end all thread
    HPRecType::clean_allHPNode_after_endAllThread();
    for (int i = 0; i < thread_num; ++i) {
        clean_rlist_after_endAllThread(i);
    }
